import { confirmOrderPaid, extractPaidCheckout, loadPayMongoCredential, verifyPayMongoSignature } from '../../server/commerce.js';
import { booleanValue, commitWrites, documentName, fieldBoolean, fieldString, getDocument, googleAccessToken, headerValue, stableId } from '../../server/server-data.js';
...
export default async function handler(req: ApiRequest, res: ApiResponse) {
  ...
  try {
    ...
    const paid = extractPaidCheckout(payload);
    ...
    if (paid.liveMode !== credential.liveMode || !(await verifyPayMongoSignature(rawBody, signature, credential.webhookSecret, credential.liveMode))) {
    ...
    if (!fieldBoolean(route, 'active')) {
      return fieldString(order, 'status') === 'paid'
        ? res.status(200).json({ ok: true, received: true, alreadyPaid: true })
        : res.status(409).json({ ok: false, error: 'Checkout route is inactive' });
    }
    ...
    await commitWrites(projectId, accessToken, [{
      update: { name: documentName(projectId, `paymentRoutes/${routeId}`), fields: { active: booleanValue(false) } },
