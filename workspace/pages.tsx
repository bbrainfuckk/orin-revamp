              ...
              {selected.id === 'meta' && (
                <>
                  ...
                  <div className={`provider-authorization ${capabilities.meta?.authorizationReady ? 'is-ready' : 'is-waiting'}`}>
                    <div><strong>{capabilities.meta?.testMode ? 'Developer test mode · app-role accounts only.' : capabilities.meta?.authorizationReady ? connections.some((connection) => connection.provider === 'meta' && connection.authorizationStatus === 'authorized') ? 'Add more Facebook Pages and Instagram accounts.' : 'Connect Facebook and Instagram in one step.' : capabilities.meta?.partnerAccessRequired ? 'Meta App Review is in progress.' : 'Meta app credentials are required.'}</strong><span>{capabilities.meta?.testMode ? 'Continue with the Facebook account assigned to this Meta app. ORIN AI will now import every Page that Meta returns for that account; public client accounts still require App Review.' : capabilities.meta?.authorizationReady ? 'Continue with Facebook. ORIN AI imports every eligible Page, links its professional Instagram account, and keeps everything already connected.' : capabilities.meta?.partnerAccessRequired ? 'The production sign-in remains locked until Meta approves ORIN AI for live client data.' : 'ORIN AI will enable Meta sign-in only after the app ID, secret, encrypted vault, callback, and webhook are configured.'}</span></div>
