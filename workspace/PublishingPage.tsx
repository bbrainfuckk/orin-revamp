import { CalendarDays, Check, Clock3, ExternalLink, Repeat2, Send, ShieldCheck, X } from 'lucide-react';
...
type SocialPost = { id: string; text: string; status: string; targets: Array<{ provider: ChannelId }>; scheduledAt?: Timestamp; recurrence: string; runNumber: number; maxRuns: number };
...
export function PublishingPage() {
  ...
  useEffect(() => {
    ...
    const stopPosts = onSnapshot(query(collection(db, 'workspaces', workspace.id, 'socialPosts'), orderBy('createdAt', 'desc'), limit(30)), (snapshot) => setPosts(snapshot.docs.map((item) => {
      ...
      return { id: item.id, text: String(data.text || ''), status: String(data.status || 'draft'), targets: parsed, scheduledAt: data.scheduledAt as Timestamp | undefined, recurrence: String(data.recurrence || 'none'), runNumber: Number(data.runNumber || 1), maxRuns: Number(data.maxRuns || 1) };
    })), () => setPosts([]));
  ...
  return <section className="workspace-page publishing-page">
    ...
    <div className="publishing-layout">
      <section className="publishing-composer">
        ...
        <label><span>Public media URL <small>optional · HTTPS</small></span><input type="url" value={mediaUrl} onChange={(event) => setMediaUrl(event.target.value)} placeholder="https://…" /></label>
        ...
        <footer>
          ...
          <button type="button" onClick={() => void submit()} disabled={busy || !canEdit || (!text.trim() && !mediaUrl.trim()) || !targets.length || (autoPost && !scheduledAt)}>{busy ? 'Publishing…' : autoPost ? 'Start autoposter' : scheduledAt ? 'Schedule everywhere' : 'Publish everywhere'} <Send /></button>
        </footer>
    ...
    <section className="publishing-ledger">
      <header><div><span>Delivery ledger</span><h2>Every outcome, accounted for.</h2></div><small>{posts.length} recent posts</small></header>
      {posts.length ? posts.map((post) => <article key={post.id}><time>{post.scheduledAt?.toDate().toLocaleString('en-PH', { month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' }) || 'Now'}</time><div><strong>{post.text || 'Media post'}</strong><small>{post.targets.map((target) => channels.find(([id]) => id === target.provider)?.[1] || target.provider).join(' · ')}</small>{post.recurrence !== 'none' && <small className="publishing-series">{post.recurrence} autoposter · run {post.runNumber} of {post.maxRuns}</small>}{deliveries.filter((item) => item.postId === post.id && item.error).map((item) => <em key={item.id}>{channels.find(([id]) => id === item.provider)?.[1]}: {niceError(item.error)}</em>)}</div><div className="publishing-ledger-actions"><span className={`publishing-status is-${post.status}`}>{post.status.replaceAll('_', ' ')}</span>{['scheduled','schedule_failed'].includes(post.status) && <button type="button" disabled={postActionId === post.id} onClick={() => void actOnPost(post.id, 'cancel')}>Cancel</button>}{['failed','partially_delivered'].includes(post.status) && <button type="button" disabled={postActionId === post.id} onClick={() => void actOnPost(post.id, 'retry')}>{postActionId === post.id ? 'Retrying…' : 'Retry'}</button>}</div></article>) : <div className="publishing-ledger__empty"><CalendarDays /><strong>Your publishing history starts here.</strong><p>Create your first campaign above. ORIN records the confirmed result from every channel.</p></div>}
    </section>
